...
# 比較演算子
a == b          # 同値       __eq__(self, other)
a != b          # 同値でない __ne__(self, other)
# a <> b        #  同値でない(!=と同じだが非推奨)
a is b          # 同一
a is not b      # 同一でない
a < b           # より小さい
...
1 < a < 5      # 比較演算子をandを使わずに組み合わせることもできる

# Javaでいうequals()(同値比較)、==(同一性比較)の関係
# NaNとの同値比較は(NaN同士でも)False
nan = float("nan")
...
print(", " + str(i) + " " + str(j))    # Pythonではループ変数やループ内で定義された変数を、ループの外でも参照できる
